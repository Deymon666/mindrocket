  ...
  return (
    ...
    >
      ...
      <motion.img
        src={mascotImg}
        alt="MindRocket"
        className="w-36 h-36 object-contain mb-6 drop-shadow-[0_0_30px_rgba(0,229,255,0.4)]"
        animate={{ y: [0, -10, 0] }}
        transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
      />

      <h1 className="text-4xl font-black text-white mb-2 text-center">
        Mind<span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00F5D4] to-[#00B4D8]">Rocket</span>
      </h1>
      ...
      <motion.button
        ...
        onClick={onLogin}
        disabled={isLoading}
        className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-bold py-4 px-6 rounded-2xl shadow-lg hover:shadow-xl transition-all disabled:opacity-60"
      >
        {isLoading ? (
      ...
      {showInstallBtn && onInstall && (
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={onInstall}
          className="mt-4 w-full flex items-center justify-center gap-2 border border-white/20 text-white/70 font-semibold py-3 px-6 rounded-2xl hover:bg-white/10 transition-all text-sm"
        >
          📲 Instalar App
        </motion.button>
      )}
