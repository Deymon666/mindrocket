...
// Guardar nombre y avatar (para usuarios nuevos)
export async function saveProfile(uid: string, name: string, avatar: string) {
  try {
    const db = getDb();
    await updateDoc(doc(db, 'users', uid), { name, avatar });
  } catch (err) {
